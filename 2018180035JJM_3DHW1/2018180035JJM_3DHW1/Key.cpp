...
void Key::update()
{
	...
	if (hFocusedHwnd)
	{
		...
		ScreenToClient(CGameFramework::GetInst()->GetMainHwnd(), &ptMouse);
...
void Key::SetEnableInput(KEY _key, bool _enable)
{

	m_arrEnabled[(UINT)_key] = _enable;
}

void Key::EnableAll(bool _enable)
{
	if (_enable)
	{
		m_arrEnabled.set();
